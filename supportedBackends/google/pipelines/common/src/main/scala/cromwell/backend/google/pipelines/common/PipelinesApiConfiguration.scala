...
class PipelinesApiConfiguration(
  ...
  val encryptedDockerCredentials = dockerCredentials flatMap {
    case PipelinesApiDockerCredentials(_, _, username, password) =>
      for {
        u <- username
        p <- password
        map = JsObject(Map("username" -> JsString(u), "password" -> JsString(p))).compactPrint
      } yield GoogleAuthMode.encryptKms(map)
  }
