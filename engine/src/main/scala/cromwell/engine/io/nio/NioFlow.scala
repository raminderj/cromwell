...
class NioFlow(parallelism: Int,
  ...
  private def withReader[A](file: Path)(f: BufferedReader => A): A = {
    ...
    tryWithResource(() => new BufferedReader(new InputStreamReader(file.mediaInputStream, Codec.UTF8.name)))(f).recoverWith({
      case failure => Failure(new IOException(s"Failed to open an input stream for ${file.pathAsString}", failure))
    }).get
  }
  ...
  private def limitFileContent(file: Path, limit: Option[Int], failOnOverflow: Boolean) = withReader(file) { reader =>
    ...
    if (failOnOverflow && limit.exists(bytesArray.length > _))
      throw new IOException(s"File $file is larger than $limit Bytes. Maximum read limits can be adjusted in the configuration under system.input-read-limits.")
    else
      limit.map(bytesArray.take).getOrElse(bytesArray)
  }
}
